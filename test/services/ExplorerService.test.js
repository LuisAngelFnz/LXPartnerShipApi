...
describe('Test Suite for class ExplorerService', ()=>{
    ...
    test('1.- Method getExplorers, pass param path, null and undefined(get with defaul path)',()=>{
        let all_explorers = ExplorerService.getExplorers(null);
        expect(all_explorers).not.toBeUndefined();

        all_explorers = ExplorerService.getExplorers();
        expect(all_explorers).not.toBeUndefined();
    });

    test('2.- Method getExplorers, with custom path', ()=>{
        let path = `/tmp/${new Date().valueOf()}`
        let all_explorers = ExplorerService.getExplorers(path);
        expect(all_explorers).toEqual([]);

        path = 'explorers.json';
        all_explorers = ExplorerService.getExplorers(path);
        expect(all_explorers).not.toBeUndefined();
    });
